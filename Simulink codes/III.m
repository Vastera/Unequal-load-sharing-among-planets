...
%               description:a flag denoting weather to round the impulsive interval, 1 is to find the closest point the nominal position, 2 (default) is to make sure all interval are identical
%% ====================== OUTPUT =======================
...
elseif round_type==2 % Make sure every interval to be identical
    inteval=round((Index(end)-Index(1))/(length(Index)-1));
    for j=1:length(Index)
        y(min([Index(1)+inteval*(j-1),length(t)]))=1;
    end
end
end
